...
public class CraftingListConsumablesPopup
{
    ...
    private void DrawMacroSection()
    {
        ...
        var precraftName = string.IsNullOrEmpty(_workingDefaultPrecraftMacroId)
            ? "无 (使用求解器)"
            : (allMacros.FirstOrDefault(m => m.Id == _workingDefaultPrecraftMacroId)?.Name ?? "(未找到宏)");
        ImGui.SetNextItemWidth(240);
        if (ImGui.BeginCombo("##PrecraftMacro", precraftName))
        {
            if (ImGui.Selectable("无 (使用求解器)", string.IsNullOrEmpty(_workingDefaultPrecraftMacroId)))
        ...
        var finalName = string.IsNullOrEmpty(_workingDefaultFinalMacroId)
            ? "无 (使用求解器)"
            : (allMacros.FirstOrDefault(m => m.Id == _workingDefaultFinalMacroId)?.Name ?? "(宏未找到)");
        ImGui.SetNextItemWidth(240);
        if (ImGui.BeginCombo("##FinalMacro", finalName))
        {
            if (ImGui.Selectable("无 (使用求解器)", string.IsNullOrEmpty(_workingDefaultFinalMacroId)))
