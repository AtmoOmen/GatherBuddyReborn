...
public class CraftingMaterialsWindow : Window
{
    ...
    public override void Draw()
    {
        if (_editor == null)
        {
            ImGui.TextColored(new Vector4(0.7f, 0.7f, 0.7f, 1), "没有打开的清单。");
            return;
        }
        ...
        if (ImGui.IsItemHovered())
            ImGui.SetTooltip("当拥有数量超过需求时显示超过 100% 的百分比"); // Show % beyond 100 when you have more than needed
        ImGui.SameLine();
        ...
        if (ImGui.BeginTable("##mat_table", colCount, tableFlags, new Vector2(0, ImGui.GetContentRegionAvail().Y)))
        {
            ...
            if (showRetainer)
                ImGui.TableSetupColumn("雇员", ImGuiTableColumnFlags.WidthFixed, countColWidth);
            ...
            if (showRetainer)
            {
                ...
                ImGui.TableHeader("雇员");
                if (ImGui.IsItemHovered())
                    ImGui.SetTooltip("雇员背包中的库存数量(通过 Allagan Tools 插件)");
            }
