...
public partial class VulcanWindow
{
    ...
    private void DrawMarketboardTab()
    {
        ...
        {
            var handle = GatherBuddy.ControllerSupport.TabNavigation.TabItem("市场板##marketboardTab", 7, 8);
            tabItem = handle;
            tabOpen = handle;
        }
        else
        ...
        using (tabItem)
        {
            ...
            if (svc == null)
            {
                ImGui.TextColored(ImGuiColors.DalamudGrey, "市场版服务不可用。");
                return;
            }
    ...
    private void DrawMarketboardHistoryList(MarketboardService svc)
    {
        ...
        {
            ImGui.TextColored(ImGuiColors.DalamudGrey, "尚无任何查询。");
            ImGui.TextColored(ImGuiColors.DalamudGrey, "在材料窗口中右键点击任意物品");
            ImGui.TextColored(ImGuiColors.DalamudGrey, "即可进行搜索。");
            return;
        }
    ...
    private void DrawMarketboardDetail(MarketboardService svc)
    {
        ...
        if (ImGui.SmallButton("Universalis##mbweb"))
        {
            ...
            catch (Exception ex) { GatherBuddy.Log.Warning($"[市场板] 打开 Universalis 链接失败: {ex.Message}"); }
        }
        ...
        if (hasErr || data == null)
        {
            ...
            ImGui.TextColored(ImGuiColors.DalamudGrey3, "或者请求失败。");
        }
        else
        {
